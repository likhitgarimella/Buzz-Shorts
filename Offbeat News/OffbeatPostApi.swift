...
class OffbeatPostApi {
    ...
    func observePosts(completion: @escaping (OffbeatModel) -> Void) {
        ...
        REF_POSTS.observe(.childAdded, with: { (snapshot) in
            ...
            if let dict = snapshot.value as? [String: Any] {
                let newPost = OffbeatModel.transformPostPhoto(dict: dict, key: snapshot.key)
                completion(newPost)
            }
    ...
    func observePost(withId id: String, completion: @escaping (OffbeatModel) -> Void) {
        
        REF_POSTS.child(id).observeSingleEvent(of: .value, with: {
            snapshot in
            
            if let dict = snapshot.value as? [String:Any] {
                let post = OffbeatModel.transformPostPhoto(dict: dict, key: snapshot.key)
                completion(post)
            }
