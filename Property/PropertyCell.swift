...
class PropertyCell: UICollectionViewCell {
    ...
    @IBOutlet weak var propertyNewsPic: UIImageView!
    ...
    var propertyFeedVC: PropertyViewController?
    
    var propertyPost: PropertyModel? {
        didSet {
            updateView()
        }
    }
    ...
    func updateView() {
        ...
        if let photoUrlString = propertyPost?.photoUrl {
            let photoUrl = URL(string: photoUrlString)
            propertyNewsPic.sd_setImage(with: photoUrl)
        }
        
        leftTagString.text = propertyPost?.leftTag
        headingString.text = propertyPost?.heading
        readTimeString.text = propertyPost?.readTime
