...
class EducationPostApi {
    ...
    func observePosts(completion: @escaping (EducationModel) -> Void) {
        ...
        REF_POSTS.observe(.childAdded, with: { (snapshot) in
            ...
            if let dict = snapshot.value as? [String: Any] {
                let newPost = EducationModel.transformPostPhoto(dict: dict, key: snapshot.key)
                completion(newPost)
            }
    ...
    func observePost(withId id: String, completion: @escaping (EducationModel) -> Void) {
        
        REF_POSTS.child(id).observeSingleEvent(of: .value, with: {
            snapshot in
            
            if let dict = snapshot.value as? [String:Any] {
                let post = EducationModel.transformPostPhoto(dict: dict, key: snapshot.key)
                completion(post)
            }
