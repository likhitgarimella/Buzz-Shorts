...
class AstroCell: UICollectionViewCell {
    ...
    @IBOutlet weak var astroNewsPic: UIImageView!
    ...
    var astroFeedVC: AstroViewController?
    
    var astroPost: AstroModel? {
        didSet {
            updateView()
        }
    }
    ...
    func updateView() {
        ...
        if let photoUrlString = astroPost?.photoUrl {
            let photoUrl = URL(string: photoUrlString)
            astroNewsPic.sd_setImage(with: photoUrl)
        }
        
        leftTagString.text = astroPost?.leftTag
        headingString.text = astroPost?.heading
        readTimeString.text = astroPost?.readTime
