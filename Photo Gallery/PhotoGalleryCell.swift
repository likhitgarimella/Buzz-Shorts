...
class PhotoGalleryCell: UICollectionViewCell {
    ...
    @IBOutlet weak var photoNewsPic: UIImageView!
    ...
    var photoFeedVC: PhotoGalleryViewController?
    
    var photoPost: PhotoModel? {
        didSet {
            updateView()
        }
    }
    ...
    func updateView() {
        ...
        if let photoUrlString = photoPost?.photoUrl {
            let photoUrl = URL(string: photoUrlString)
            photoNewsPic.sd_setImage(with: photoUrl)
        }
        
        leftTagString.text = photoPost?.leftTag
        headingString.text = photoPost?.heading
        readTimeString.text = photoPost?.readTime
