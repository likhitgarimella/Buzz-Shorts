...
class FoodCell: UICollectionViewCell {
    ...
    @IBOutlet weak var foodNewsPic: UIImageView!
    ...
    var foodFeedVC: FoodViewController?
    
    var foodPost: FoodModel? {
        didSet {
            updateView()
        }
    }
    ...
    func updateView() {
        ...
        if let photoUrlString = foodPost?.photoUrl {
            let photoUrl = URL(string: photoUrlString)
            foodNewsPic.sd_setImage(with: photoUrl)
        }
        
        leftTagString.text = foodPost?.leftTag
        headingString.text = foodPost?.heading
        readTimeString.text = foodPost?.readTime
