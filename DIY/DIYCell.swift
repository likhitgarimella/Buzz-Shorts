...
class DIYCell: UICollectionViewCell {
    ...
    @IBOutlet weak var diyNewsPic: UIImageView!
    ...
    var diyFeedVC: DIYViewController?
    
    var diyPost: DIYModel? {
        didSet {
            updateView()
        }
    }
    ...
    func updateView() {
        ...
        if let photoUrlString = diyPost?.photoUrl {
            let photoUrl = URL(string: photoUrlString)
            diyNewsPic.sd_setImage(with: photoUrl)
        }
        
        leftTagString.text = diyPost?.leftTag
        headingString.text = diyPost?.heading
        readTimeString.text = diyPost?.readTime
